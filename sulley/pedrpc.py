...
class Client(object):
  ...
  def __getattr__(self, method_name):
    """Routine is called by default when a requested attribute has no definition.

    Unfortunately __getattr__ only passes the requested method name and not the arguments.
    So we extend the functionality with a little lambda magic to the routine method_missing().
    Which is actually how Ruby handles missing methods by default ... with arguments.
    Now we are just as cool as Ruby.

    @type  method_name: String
    @param method_name: The name of the requested and undefined attribute
                        (or method in our case).

    @rtype:  Lambda
    @return: Lambda magic passing control (and in turn the arguments we want) to
                      self.method_missing().
    """
  ...
  def __pickle_recv(self):
    """Routine is used for marshaling arbitrary data from the PyDbg server.

    We can send pretty much anything here.  For example a tuple containing integers, strings,
    arbitrary objects and structures. Our "protocol" is a simple length-value protocol where
    each datagram is prefixed by a 4-byte length of the data to be received.
    ...
    try:
      """TODO: this should NEVER fail, but alas, it does and for the time being i can't figure out why.
         it gets worse. you would think that simply returning here would break things, but it
         doesn't.  Gotta track this down at some point."""
  ...
  def __pickle_send(self, data):
    """Routine is used for marshaling arbitrary data to the PyDbg server.

    We can send pretty much anything here.
    For example a tuple containing integers, strings, arbitrary objects and structures.
    Our "protocol" is a simple length-value protocol where each datagram is prefixed by a 4-byte
    length of the data to be received.

    @type  data: Mixed
    @param data: Data to marshal and transmit. Data can *pretty much* contain anything you
                throw at it.
...
class Server(object):
  ...
  def __pickle_recv(self):
    """Used for marshaling arbitrary data from the PyDbg server.
  ...
  def __pickle_send(self, data):
    """Used for marshaling arbitrary data to the PyDbg server.

    We can send pretty much anything here.
    For example a tuple containing integers, strings, arbitrary objects and structures.
    Our "protocol" is a simple length-value protocol where each datagram is prefixed by a
    4-byte length of the data to be received.
  ...
  def serve_forever(self):
    ...
    while 1:
      ...
      self.__debug("accepted connection from %s:%d" % (
