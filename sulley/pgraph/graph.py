...
class graph(object):
    ...
    def add_edge(self, edge, prevent_dups=True):
        ...
        Ensures a node exists for both the source and destination of the edge.

        @type  edge:         pGRAPH Edge
        @param edge:         Edge to add to graph
        @type  prevent_dups: Boolean
        @param prevent_dups: (Optional, Def=True) Flag controlling whether or not the addition of
                                duplicate edges is ok
        """
        if prevent_dups:
    ...
    def add_node(self, node):
        ...
        Ensures a node with the same id does not already exist in the graph.
    ...
    def find_cluster(self, attribute, value):
        ...
        """
        for clstr in self.clusters:
            if hasattr(clstr, attribute):
                if getattr(clstr, attribute) == value:
                    return clstr
    ...
    def find_edge(self, attribute, value):
        ...
        # step through all the edges looking for the given attribute/value pair.
            ...
            for edg in self.edges.values():
                if hasattr(edg, attribute):
                    if getattr(edg, attribute) == value:
                        return edg
        return None
    ...
    def graph_down(self, from_node_id, max_depth=-1):
        ...
        @param max_depth:    (Optional, Def=-1) Number of levels to include in down graph
                                (-1 for infinite)
        ...
        for level in levels_to_process:
            ...
            for nde in level:
                down_graph.add_node(copy.copy(nde))

                for edg in self.edges_from(nde.id):
                    to_add = self.find_node("id", edg.dst)

                    if not down_graph.find_node("id", edg.dst):
                        next_level.append(to_add)

                    down_graph.add_node(copy.copy(to_add))
                    down_graph.add_edge(copy.copy(edg))
    ...
    def graph_intersect(self, other_graph):
        ...
        """
        for nde in self.nodes.values():
            if not other_graph.find_node("id", nde.id):
                self.del_node(nde.id)

        for edg in self.edges.values():
            if not other_graph.find_edge("id", edg.id):
                self.del_edge(edg.id)
        return self
    ...
    def graph_proximity(self, center_node_id, max_depth_up=2, max_depth_down=2):
        ...
        @param max_depth_up:   (Optional, Def=2) Number of upward levels to include in proximity
                                graph
        @type  max_depth_down: Integer
        @param max_depth_down: (Optional, Def=2) Number of downward levels to include in proximity
                                graph
        @rtype:  pgraph.graph
    ...
    def graph_up(self, from_node_id, max_depth=-1):
        ...
        @type  max_depth:    Integer
        @param max_depth:    (Optional, Def=-1) Number of levels to include in up graph
                                (-1 for infinite)
        @rtype:  pgraph.graph
        ...
        for level in levels_to_process:
            ...
            if current_depth > max_depth and max_depth != -1:
                break
            for nde in level:
                up_graph.add_node(copy.copy(nde))
                for edg in self.edges_to(node.id):
                    to_add = self.find_node("id", edg.src)
                    if not up_graph.find_node("id", edg.src):
                        next_level.append(to_add)
                    up_graph.add_node(copy.copy(to_add))
                    up_graph.add_edge(copy.copy(edg))
            if next_level:
                levels_to_process.append(next_level)
            current_depth += 1
        return up_graph
    ...
    def render_graph_gml(self):
        ...
        for nde in self.nodes.values():
            gml += nde.render_node_gml(self)

        # add the edges to the GML definition.
        for edg in self.edges.values():
            gml += edg.render_edge_gml(self)
    ...
    def render_graph_graphviz(self):
        ...
        for nde in self.nodes.values():
            dot_graph.add_node(nde.render_node_graphviz(self))

        for edg in self.edges.values():
            dot_graph.add_edge(edg.render_edge_graphviz(self))
        return dot_graph
    ...
    def render_graph_udraw(self):
        ...
        for nde in self.nodes.values():
            udraw += nde.render_node_udraw(self)
            udraw += ','
    ...
    def render_graph_udraw_update(self):
        ...
        for nde in self.nodes.values():
            udraw += nde.render_node_udraw_update()
            udraw += ','

        for edg in self.edges.values():
            udraw += edg.render_edge_udraw_update()
            udraw += ','
    ...
    def update_node_id(self, current_id, new_id):
        ...
        This routine will correctly update the edges as well.
        ...
        # update the node.
        nde = self.nodes[current_id]
        del self.nodes[current_id]
        nde.id = new_id
        self.nodes[nde.id] = nde

        # update the edges.
        for edg in [edg for edg in self.edges.values() if current_id in (edg.src, edg.dst)]:
            del self.edges[edg.id]

            if edg.src == current_id:
                edg.src = new_id
            if edg.dst == current_id:
                edg.dst = new_id

            edg.id = (edge.src << 32) + edg.dst

            self.edges[edg.id] = edg
