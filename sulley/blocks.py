...
class request(pgraph.node):
    ...
    def __init__(self, name):
        ...
        Can hold any block structure or primitive. This can
        essentially be thought of as a super-block, root-block, daddy-block or whatever other alias
        you prefer.
        ...
        self.callbacks = {}  # dictionary of list of sizers / checksums that were unable to render
    ...
    def push(self, item):
        ...
        If no block is open, the item goes onto the request
        stack. otherwise, the item goes onto the last open blocks stack.
        """
    ...
    def render(self):
        ...
        def update_size(stack, name):
            """Walk each block update the size."""
            blocks = []
...
class block(object):
    ...
    def __init__(
        ...
        Can contain primitives, sizers, checksums or other blocks.
        ...
        @param encoder:     (Optional, def=None) Optional pointer to a function to pass rendered
                            data to prior to return
        @type  dep:         String
        @param dep:         (Optional, def=None) Optional primitive whose specific value this block
                            is dependant on
        @type  dep_value:   Mixed
        @param dep_value:   (Optional, def=None) Value that field "dep" must contain for block to be
                            rendered
        @type  dep_values:  List of Mixed Types
        @param dep_values:  (Optional, def=[]) Values that field "dep" may contain for block to be
                            rendered
    ...
    def mutate(self):
        ...
        if self.group:
            ...
            cur_idx = self.request.names[self.group].values[self.group_idx]
            self.request.names[self.group].value = cur_idx
            ...
            if not mutated:
                ...
                if self.group_idx == group_count:
                    ...
                    original_grp_val = self.request.names[self.group].original_value
                    self.request.names[self.group].value = original_grp_val
        ...
        """if this block is dependant on another field, then manually update that fields value
        appropriately while we mutate this block. we'll restore the original value of the field
        prior to continuing."""
    ...
    def num_mutations(self):
        ...
        # if this block is associated with a group, then multiply out the number of mutations.
        if self.group:
    ...
    def render(self):
        ...
        Subsequent blocks recursively render their stacks.
        """
        ...
        if type(self.algorithm) is str:
            if self.algorithm == "crc32":
                ...
                raise sex.SullyRuntimeError(
                    "INVALID CHECKSUM ALGORITHM SPECIFIED: %s" % self.algorithm)
        else:
...
class repeat(object):
    """This block type is a hybrid between a block and a primitive (it can be fuzzed).

    Theuser does not need to be wary of this fact.
    """
    ...
    def __init__(
        ...
        """Repeat the rendered contents of the block from min_reps to max_reps counting by step.
        ...
        @param variable:   (Optional, def=None) Repititions will be derived from this variable,
                            disables fuzzing
        @type  fuzzable:   Boolean
        @param fuzzable:   (Optional, def=True) Enable/disable fuzzing of this primitive
        @type  name:       String
        @param name:       (Optional, def=None) Specifying a name gives you direct access
                            to a primitive
        """
        self.block_name = block_name
        self.request = request
        ...
        if self.block_name not in self.request.names:
            raise sex.SullyRuntimeError(
                "CAN NOT ADD REPEATER FOR NON-EXISTANT BLOCK: %s" % self.block_name)
        ...
        if self.variable is None and self.max_reps is None:
            raise sex.SullyRuntimeError(
                "REPEATER FOR BLOCK %s DOES NOT HAVE A MIN/MAX OR VARIABLE BINDING" % (
                    self.block_name))
        ...
        if self.variable and not isinstance(self.variable, primitives.bit_field):
            print(self.variable)
            raise sex.SullyRuntimeError(
                "ATTEMPT TO BIND THE REPEATER FOR BLOCK %s TO A NON INTEGER PRIMITIVE" % (
                    self.block_name))

        # if not binding variable was specified, propogate with the repetition counts.
    ...
    def mutate(self):
        ...
        If variable-bounding is
        specified then fuzzing is implicitly disabled. Instead, the render() routine will properly
        ...
        if self.block_name not in self.request.closed_blocks:
            raise sex.SullyRuntimeError(
                "CAN NOT APPLY REPEATER TO UNCLOSED BLOCK: %s" % self.block_name)

        # if we've run out of mutations, raise the completion flag.
        if self.mutant_index == self.num_mutations():
            self.fuzz_complete = True

        # if fuzzing was disabled or complete, and mutate() called, ensure original is restored.
        ...
        # set the current value as a multiple of the block based on the current fuzz library count.
...
class size(object):
    """This block type is kind of special in that it is a hybrid between a block and a primitive.

    (it can be fuzzed). The user does not need to be wary of this fact.
    """
    ...
    def __init__(
        ...
        self.bit_field = primitives.bit_field(
            0,
            self.length * 8,
            endian=self.endian,
            format=self.format,
    ...
    def render(self):
        ...
        # if the sizer is fuzzable and we have not yet exhausted the the possible bit field values,
        # use the fuzz value.
