...
class connection(pgraph.edge):
    ...
    def __init__(self, src, dst, callback=None):
        """Extend pgraph.edge with a callback option.

        This allows us to register a function to call between node
        transmissions to implement functionality such as challenge response systems.
        The callback method must follow this prototype::

            def callback(session, node, edge, sock)

        Where node is the node about to be sent, edge is the last edge along the current fuzz path
        to "node", session is a pointer to the session instance which is useful for snagging data
        such as sesson.last_recv which contains the data returned from the last socket transmission
        and sock is the live socket. A callback is also useful in situations where, for example,
        the size of the next packet is specified in the first packet.
        ...
        @param callback: (Optional, def=None) Callback function to pass received data to between
                          node xmits
        """
        # run the parent classes initialization routine first.
        pgraph.edge.__init__(self, src, dst)
...
class session(pgraph.graph):
    ...
    def __init__(
        ...
        """Extend pgraph.graph and provides a container for architecting protocol dialogs.
        ...
        @type  timeout:            Float
        @kwarg timeout:            (Optional, def=5.0) Seconds to wait for a send/recv prior to
                                    timing out
        @type  restart_interval:   Integer
        @kwarg restart_interval    (Optional, def=0) Restart the target after n test cases, disable
                                    by setting to 0
        @type  crash_threshold:    Integer
        @kwarg crash_threshold     (Optional, def=3) Maximum number of crashes allowed before a node
                                    is exhaust
        @type  restart_sleep_time: Integer
        @kwarg restart_sleep_time: Optional, def=300) Time in seconds to sleep when target can't be
                                    restarted
        @type  web_port:	   Integer
        @kwarg web_port:           (Optional, def=26000) Port for monitoring fuzzing campaign via a
                                    web browser
        """
        # run the parent classes initialization routine first.
        ...
        try:
            ...
            self.import_file()
        # create a root node. we do this because we need to start fuzzing from a single point and
        # the user may want to specify a number of initial requests.
    ...
    def add_node(self, node):
        ...
        We overload this routine to automatically generate and assign ID whenever a node is added.
    ...
    def connect(self, src, dst=None, callback=None):
        ...
        Process in between transmissions of the source and destination request. Leverage this
        functionality to handle situations such as challenge response systems. The session class
        maintains a top level node that all initial requests must be connected to. Example::

            sess = sessions.session()
            sess.connect(sess.root, s_get("HTTP"))

        If given only a single parameter, sess.connect() will default to attaching the supplied
        ...
        such as sesson.last_recv which contains the data returned from the last socket transmission
        and sock is the live socket. A callback is also useful in situations where, for example,
        the size of the next packet is specified in the first packet. As another example,
        if you need to fill in the dynamic IP address of the target register a callback that snags
        the IP from sock.getpeername()[0].
        ...
        """
        # if only a source was provided, then make it the destination and set the source to the root
        if not dst:
            dst = src
            src = self.root
    ...
    def export_file(self):
        ...
        data["tls_version"] = self.tls_version
        with open(self.session_filename, "wb+") as fh:
    ...
    def fuzz(self, this_node=None, path=[]):
        ...
        for edge in self.edges_from(this_node.id):
            ...
            # we keep track of edges as opposed to nodes because if there is more then one path
            # through a set of given nodes we don't want any ambiguity.
            path.append(edge)
            ...
            self.logger.info("fuzzed %d of %d total cases" % (
            ...
            while not done_with_fuzz_node:
                ...
                # note: when mutate() returns False, the node has been reverted to the default
                # (valid) state.
                ...
                if self.total_mutant_index > self.skip:
                    self.logger.info("fuzzing %d of %d" % (
                        self.fuzz_node.mutant_index, num_mutations))

                    # attempt to complete a fuzz transmission. keep trying until we are successful,
                    # whenever a failure occurs, restart the target.
                    while 1:
                        ...
                        # if the user registered a pre-send function, pass it the sock and
                        # let it do the deed.
                        try:
                            self.pre_send(sock)
                        except Exception, e:
                        ...
                        # send out valid requests for each node in the current path up to the node
                        # we are fuzzing.
                        try:
                            for e in path[:-1]:
                        ...
                        # if we reach this point the send was successful
                        break

                    # if the user registered a post-send function, pass it the sock
                    # we do this outside the try/except loop if our fuzz causes a crash then
                    # the post_send() will likely fail and we don't want to sit in an endless loop.
                    try:
                        self.post_send(sock)
                    except Exception, e:
    ...
    def import_file(self):
        ...
        @see: export_file()
        """
        with open(self.session_filename, "rb") as fh:
    ...
    #def log (self, msg, level=1):
        '''
        If the supplied message falls under the current log level, print the specified message to
        screen.
...
#
    ...
    def num_mutations(self, this_node=None, path=[]):
        ...
        for inline comments. The member varialbe self.total_num_mutations is updated appropriately
        by this routine.
    ...
    def poll_pedrpc(self, target):
        ...
        if target.netmon:
            ...
            self.logger.info("netmon captured %d bytes for test case #%d" % (
                bytes, self.total_mutant_index))
            self.netmon_results[self.total_mutant_index] = bytes

        # check if our fuzz crashed the target. procmon.post_send() returns False if the
        # target access violated.
        if target.procmon and not target.procmon.post_send():
            ...
            self.crashing_primitives[self.fuzz_node.mutant] = self.crashing_primitives.get(
                self.fuzz_node.mutant, 0) + 1
            ...
            msg += "type: %s, default value: %s" % (
                self.fuzz_node.mutant.s_type, self.fuzz_node.mutant.original_value)
            ...
            if self.crashing_primitives[self.fuzz_node.mutant] >= self.crash_threshold:
                ...
                if not isinstance(self.fuzz_node.mutant, primitives.group):
                    if not isinstance(self.fuzz_node.mutant, blocks.repeat):
                        ...
                        self.logger.warning(
                            "crash threshold reached for this primitive, exhausting %d mutants." % (
                                skipped))
            ...
            if self.restart_target(target, stop_first=False) is False:
    ...
    def restart_target(self, target, stop_first=True):
        ...
        If a VMControl is available revert the snapshot, if a process monitor is available
        ...
        # otherwise all we can do is wait a while for the target to recover on its own.
            ...
            self.logger.error(
                "no vmcontrol or procmon channel available ... sleeping for %d seconds" % (
                    self.restart_sleep_time))
    ...
    def server_init(self):
        ...
        Called by fuzz() on first run (not on recursive re-entry) to initialize variables,
        web interface, etc...
        """
        self.total_mutant_index = 0
        ...
        # TODO: should wait for the end of the test case, and stop gracefully netmon and procmon
    ...
    def transmit(self, sock, node, edge, target):
        ...
        # if the edge has a callback, process it. the callback has the option to render the node,
        ...
        if self.proto == socket.SOCK_DGRAM:
            ...
            # - See http://stackoverflow.com/questions/25841/maximum-buffer-length-for-sendto
            MAX_UDP = 65507
        ...
        if self.proto == (socket.SOCK_STREAM or socket.SOCK_DGRAM):
            # TODO: might have a need to increase this at some point.
            # (possibly make it a class parameter)
            try:
    ...
    def view_index(self):
        ...
        response = """
                    ...
                    <tr bgcolor="#333333">
                        ...
                        <td align=right><div style="font-weight: bold; font-size: 20px;">%(status)s
                        </div></td>
                    </tr>
                    <tr bgcolor="#111111">
        ...
        for key in keys:
            ...
            response += '<tr><td class="fixed"><a href="/view_crash/%d">%06d</a></td><td>%s</td>' \
                '<td align=right>%s</td></tr>' % (key, key, val.split("\n")[0], bytes)
        ...
        if self.session.fuzz_node:
            ...
            progress_current = float(self.session.fuzz_node.mutant_index) / float(
                self.session.fuzz_node.num_mutations())
            num_bars = int(progress_current * 50)
            progress_current_bar = "[" + "=" * num_bars + "&nbsp;" * (50 - num_bars) + "]"
            progress_current = "%.3f%%" % (progress_current * 100)

            progress_total = float(self.session.total_mutant_index) / float(
                self.session.total_num_mutations)
...
class web_interface_server(BaseHTTPServer.HTTPServer):
    ...
    def __init__(self, server_address, req_hand, session):
        """"Initialize."""
        BaseHTTPServer.HTTPServer.__init__(self, server_address, req_hand)
        self.RequestHandlerClass.session = session


class web_interface_thread(threading.Thread):
    """Thread component for web interface."""
    ...
    def run(self):
        """Run a thread."""
        self.server = web_interface_server(
            ('', self.session.web_port), web_interface_handler, self.session)
